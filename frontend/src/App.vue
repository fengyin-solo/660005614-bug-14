<template>
  <div class="app-root">
    <header class="top-bar">
      ...
      <div class="tools">
        <el-radio-group v-model="activeView" size="small">
          <el-radio-button label="dag">DAG 编排</el-radio-button>
          <el-radio-button label="records">明细导出</el-radio-button>
        </el-radio-group>
        <template v-if="activeView === 'dag'">
    ...
    <DetailExport v-if="activeView === 'records'" />
...
import DetailExport from './components/DetailExport.vue'
import { useDAGStore } from './store/dag'
const store = useDAGStore()
const wfName = ref('data-pipeline')
const activeView = ref<'dag' | 'records'>('dag')
...
.top-bar{display:flex;justify-content:space-between;align-items:center;padding:10px 20px;background:#1a1a2e;border-bottom:1px solid #2a2a4a;gap:12px}
